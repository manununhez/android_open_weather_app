...
public final class OpenWeatherDateUtils {
    ...
    public static String getTimeAgo(long timeInSecs) {
        if (timeInSecs < 1000000000000L) {
            // if timestamp given in seconds, convert to millis
            timeInSecs *= 1000;
        }

        long now = getUnixTimeNowInSecs() * 1000;
        if (timeInSecs > now || timeInSecs <= 0) {
            return null;
        }

        // TODO: localize
        final long diff = now - timeInSecs;
    ...
    public static long getNormalizedUtcMs(long utcMillis) {
...
//        long utcNowMillis = System.currentTimeMillis();
         ...
         * into this method so it can determine changes to account for daylight savings time.
         */
        long gmtOffsetMillis = currentTimeZone.getOffset(utcMillis);
         ...
         * January 1, 1970 (GMT) time.
         */
        long timeSinceEpochLocalTimeMillis = utcMillis + gmtOffsetMillis;
