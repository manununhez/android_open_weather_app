...
import com.openweather.challenge.openweatherapp.location.CurrentFusedLocationListener;
...
public class AddCityActivity extends AppCompatActivity implements SearchView.OnQueryTextListener, SearchWeatherItemClickCallback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AddCityViewModelFactory factory = InjectorUtils.provideAddCityViewModelFactory(Objects.requireNonNull(this).getApplicationContext());
        mViewModel = ViewModelProviders.of(this, factory).get(AddCityViewModel.class);

        mProgressBar = findViewById(R.id.progressBar);
        tvCountResults = findViewById(R.id.tvCountResults);

        Button btnCurrenLocation = findViewById(R.id.btnCurrentLocation);
            ...
            if (response != null) {
                if (response.status.equals(Resource.Status.SUCCESS)) {
                    ...
                    if (ifBtnLocationHasBeenPressed) { //we read the values of this observer only if the user has selected to get current location
                        ...
                        Toast.makeText(this, "Added " + response.data.getName() + " as a new city.", Toast.LENGTH_SHORT).show();
    ...
    private void initRecyclerView() {
        mRecyclerView = findViewById(R.id.rvSearch);
    ...
    private void instantiateLoading() {
        loadingDialog = new LoadingDialog(this, getString(R.string.title_loading));
    ...
    private void getLocationUpdates() {
        CurrentFusedLocationListener.getInstance(this).observe(this, new Observer<Location>() {
            @Override
            public void onChanged(@Nullable Location location) {
                if (location != null) {
                    ...
                    CurrentFusedLocationListener.getInstance(AddCityActivity.this).removeObserver(this);//We receive the coords only once and then we cancel the observer to stop receiving location
                }
            }
        });
    }
    ...
    private void checkLocationServices() {
        ...
        LocationManager lm = (LocationManager) Objects.requireNonNull(this).getSystemService(Context.LOCATION_SERVICE);
        ...
        if (!gps_enabled && !network_enabled) {
            // notify user
            dismissLoading();
            Utils.getAlertDialogWithChoice(this, getResources().getString(R.string.title_alert_dialog), getResources().getString(R.string.gps_network_not_enabled),
                    ...
                    }, this.getString(R.string.btn_cancel), new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialogInterface, int i) {
    ...
    public void onClick(WeatherEntity weatherEntity) {
        Toast.makeText(this, "Added " + weatherEntity.getName() + " as a new city.", Toast.LENGTH_SHORT).show();
